      ...
      <div className="flex-1 min-w-0">
        ...
        {user.school?.name && (
          <div className="flex items-center gap-1 mt-1">
            <span className="text-xs text-gray-600 truncate">{user.school.name}</span>
          </div>
        )}
      </div>
    ...
    <div className="mb-4 p-3 bg-gradient-to-r from-pastel-green-50 to-emerald-50 border border-pastel-green-200 rounded-lg">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-pastel-green-700">
          이 지역에서 내 순위: <span className="font-bold">#{rank}</span>
        </span>
      </div>
    </div>
  );
...
export default function RegionRankingPage() {
  ...
  const loadRankings = async (reset = false) => {
    try {
      ...
      console.error('지역 랭킹 로드 오류:', error);
      setState(prev => ({ 
        ...prev, 
        isLoading: false, 
  ...
  const isCurrentUserInRegion = user && user.regions?.sido === decodeURIComponent(sido) && user.regions?.sigungu === decodeURIComponent(sigungu);
    ...
    <div className="container mx-auto px-3 py-4 pb-20">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-2 flex items-center gap-2">
          ...
          {decodeURIComponent(sido)} {decodeURIComponent(sigungu)} 랭킹
        </h1>
        <p className="text-sm text-gray-600">
          이 지역에 속한 사용자들의 경험치 순위입니다
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {decodeURIComponent(sido)} {decodeURIComponent(sigungu)} 지역 사용자 순위
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* 현재 사용자 순위 표시 */}
